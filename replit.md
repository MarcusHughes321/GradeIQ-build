...
- **Admin Finance Tab**: `GET /api/admin/financials` returns P&L: gross MRR (from RC tier counts × known GBP prices), platform fee (configurable — Apple SBP=15%, standard=30%), RC fee (1% if MRR > $2500), net MRR, real AI costs from `ai_cost_log`, Replit cost (editable via `admin_settings` table key `replit_monthly_gbp`), profit, margin %. Also: `dev_cost_to_date_gbp` (total invested building app), `platform_fee_pct` (editable %). Breakeven months calculated when dev cost > 0 and profitable. Admin analytics screen has Stats | Finance tab switcher. Finance tab shows Revenue card (editable platform fee %), Costs card (editable Replit cost, editable dev cost to date), P&L card with big profit number + breakeven months, per-grade economics. **Photo backup for existing users**: on startup, `retroactiveImageUpload()` runs after history sync to upload any local images not yet backed up to App Storage (up to 30 grades, fire-and-forget).
- **Investment to Date breakdown**: `costsToDate` section in financials response. Set `months_building` once → backend auto-calculates: Replit (months × monthly rate), Apple Developer (£99 × full years), Google Play (£19.82 one-time). AI API costs pulled automatically from `ai_cost_log` all-time total. `other_costs_gbp` for manual one-offs. P&L card shows Investment Payback with formula (total ÷ monthly profit = months). `admin_settings` keys: `months_building`, `other_costs_gbp`.
