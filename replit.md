...
- **Admin Finance Tab**: `GET /api/admin/financials` returns P&L: gross MRR (from RC tier counts × known GBP prices), platform fee (15%), RC fee (1% if MRR > $2500), net MRR, real AI costs from `ai_cost_log`, Replit cost (editable via `admin_settings` table key `replit_monthly_gbp`), profit, margin %. Admin analytics screen has Stats | Finance tab switcher. Finance tab shows Revenue card, Costs card (with inline-editable Replit cost), P&L card with big profit number, per-grade economics.
