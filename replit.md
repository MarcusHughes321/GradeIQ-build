...
- **Grading History**: Stored locally via AsyncStorage AND synced to server (`grading_history` DB table). Keyed by RC anonymous user ID (no PII — GDPR safe). Grade results (not images) are backed up so they survive app reinstalls. Synced in both directions on startup: server records missing locally are restored; local records missing on server are uploaded. Uploads happen fire-and-forget after each grade. Deletes propagate to server. Images stay local-only (too large for DB).
