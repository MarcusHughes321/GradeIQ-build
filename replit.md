...
- **User Features**:
    ...
    - **Bulk Grading**: Up to 20 cards simultaneously (Quick Grade only). **Full parity with single-card grades** on three axes: (1) **cloud photo backup** — the server stores full-res originals per card during grading (`storeGradeOriginal`) and the image IDs ride back inside each result; the client sets `frontImageUrl`/`backImageUrl` from those IDs so cards are never flagged "not backed up" and restore on reinstall; (2) **server history sync** — graded cards are synced via `uploadBulkGradings` then per-card `linkGradingImages`; (3) **kill-safe delivery** — bulk results persist to `grading_jobs.result` via `completeJobInDb` (shape `{ results, totalCards }`) and `/api/pending-grades` now includes `type='bulk'`; on launch the client recovery loop saves each completed card silently (never opening the single-result modal) and acknowledges once. The client sends `rcUserId`/`stableUserId` in the POST, and the server records canonical usage per successful card (`recordServerUsage`), so free-tier counts survive reinstall. Local history stores the original file URIs (not base64).
    - **Subscription Model**: Tiered access to features (Free, Grade Curious, Grade Enthusiast, Grade Obsessed) managed by RevenueCat.
    - **Reinstall-safe Usage Counts**: `usage_tracking` table now has a `stable_user_id` column (with a partial unique index on `stable_user_id + year_month`). All three grading endpoints write to and read from this canonical stable-ID row, so free-tier grade counts survive a full app reinstall without resetting. The client sends `stableUserId` alongside `rcUserId` in every grade submission. On startup, `syncServerUsage` is called twice: once immediately with the RC user ID, then again with the stable ID once it is loaded from Keychain, ensuring the display count reflects the real server value.
...
- **Grading History**: Stored locally via AsyncStorage AND synced to server (`grading_history` DB table). Keyed by **stable UUID** (persists across reinstalls) AND RC anonymous user ID. **Stable UUID** (`lib/stable-user-id.ts`): generated once on first launch, stored in iOS Keychain (survives reinstall since iOS 10.3+) and AsyncStorage (covered by Android Auto Backup / Google Drive). On startup, `POST /api/history/claim` links the stable UUID to existing rows and re-keys rc_user_id after reinstall. Synced bidirectionally on startup using stableId. Deletes propagate to server. **Photo backup**: Front + back images uploaded to Replit Object Storage (`.private` bucket) at 400px/JPEG 60% — stored as `front_image_url`/`back_image_url` in DB. On reinstall, images restore from server URL via `GET /api/grading-image/:uuid`. Client falls back to `frontImageUrl`/`backImageUrl` in `SavedGrading` when local image is empty.
- **AI Cost Tracking**: Every Claude call logs `(month, mode, model, input_tokens, output_tokens, cost_usd)` to `ai_cost_log` table. Used by Finance tab for real spend data. Modes tracked: `quick`, `deep`, `collection`. Pricing: claude-sonnet-4-6 at $3/M input, $15/M output.
- **Admin Finance Tab**: `GET /api/admin/financials` returns P&L: gross MRR (from RC tier counts × known GBP prices), platform fee (configurable — Apple SBP=15%, standard=30%), RC fee (1% if MRR > $2500), net MRR, real AI costs from `ai_cost_log`, monthly costs (Replit + PokeTrace + AI), profit, margin %. Monthly recurring costs: Replit subscription (editable `replit_monthly_gbp`), PokeTrace API (editable `poketrace_monthly_gbp` default £15). Breakeven months calculated when total invested > 0 and profitable. Admin analytics screen has Stats | Finance tab switcher. Finance tab: Revenue card (editable platform fee %), Costs card (editable Replit monthly + PokeTrace monthly), Investment to Date card (individual editable fields per cost), P&L card with big profit + breakeven, per-grade economics. **Photo backup for existing users**: on startup, `retroactiveImageUpload()` runs after history sync to upload any local images not yet backed up to App Storage (up to 30 grades, fire-and-forget).
- **Investment to Date breakdown**: `costsToDate` section in financials response. Individual real amounts (not calculated from months): `replit_total_gbp` (actual bank total £870.42), `apple_licence_gbp` (£75), `google_play_gbp` (£20), `poketrace_monthly_gbp` × `poketrace_months_used` (£15 × 3 = £45), `other_costs_gbp` (£40). AI API costs pulled automatically from `ai_cost_log` all-time total. All fields editable inline. P&L card shows Investment Payback with formula (total ÷ monthly profit = months). `admin_settings` keys: `replit_total_gbp`, `apple_licence_gbp`, `google_play_gbp`, `poketrace_monthly_gbp`, `poketrace_months_used`, `other_costs_gbp`, `replit_monthly_gbp`, `platform_fee_pct`.
- **Card Advisor**: AI-powered chat tool for card market analysis — grading economics, deal evaluation, investment outlook, liquidity. **2-phase architecture**: Phase 1 = user describes a card in natural language → `POST /api/card-advisor/search` queries `card_catalog` using keyword scoring (name match ×3, set_name match ×1, rarity stop-words filtered) → returns up to 8 matching cards for user to tap the exact one. Phase 2 = selected card locked in → `POST /api/card-advisor/advice` fetches real PokeTrace graded prices + TCGPlayer/Cardmarket raw price → single Claude Haiku call with all price data in system prompt → returns conversational advice + prices block. Follow-up messages re-use same endpoint with rolling history. Card Advisor deeplinks to full Card Profit screen. Entry: Grade hub → Collection Tools → Card Advisor. Screen: `app/deal-advisor.tsx`. AI costs logged to `ai_cost_log` as mode `deal_advisor` (claude-haiku-4-5).
...
- **Collection Scan** *(currently hidden from the UI — entry point removed from the Grade hub; screen + backend code retained so it can be re-enabled)*: Lightweight condition-check feature (free). Scans front+back of multiple cards using Claude Haiku; returns condition (Mint/NM/LP/Played/HP/Damaged), card identity, and raw price with condition multiplier applied. Generates a CSV report for seller transactions. Rate-limited silently (100/session, 300/month) via `collection_scan_usage` DB table. Screens: `app/collection-scan.tsx`, `app/collection-results.tsx`. Backend routes: `POST/GET /api/collection/job`, `PUT /api/collection/job/:jobId/card/:idx`.
