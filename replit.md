...
- **Image Processing**:
    ...
    - **AI Card Boundary Detection**: Two-tier system. (1) **Claude-first (primary)**: `detectRawCardBoundsWithAI()` sends image to Claude Sonnet 4-6 with a step-by-step prompt to identify card edges as image-% coordinates. Used by `/api/detect-bounds` endpoint and as primary method in the centering tool. (2) **Sobel fallback**: Multi-resolution Sobel gradient approach (coarse 200px → fine 600px) using line profile + rectangle fitting, used when Claude fails. The grading prompt (Step 6) also instructs Claude to return `frontCardBounds`/`backCardBounds` as part of the grading JSON — when valid, these are used directly instead of running Sobel post-grading, so centering lines always match AI measurements. Validity check: `isValidCardBounds()` requires span 30-94% in each dimension, within 1-99% bounds.
