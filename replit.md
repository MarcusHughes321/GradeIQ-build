...
- **Deal Advisor**: AI-powered chat tool for evaluating buying/selling deals. User describes a deal in natural language (e.g. "Someone offered me £400 for a PSA 10 Base Set Charizard"). Two-step Claude pipeline: (1) parse + identify cards with structured JSON, (2) look up real eBay last-sold prices via PokeTrace + card images from `card_catalog`, then advise with price context. Returns: conversational verdict, per-card tiles (image, grade badge, market value, avg7d, sale count), combined market total vs offered price, deal score % (green/amber/red). Entry: Grade hub → Collection Tools → Deal Advisor. Screen: `app/deal-advisor.tsx`. Backend: `POST /api/deal-advisor`. AI costs logged to `ai_cost_log` as mode `deal_advisor`.
- **Card Advisor**: Renamed and expanded from Deal Advisor. Now covers general market research, investment outlook, liquidity analysis, and grading economics — not just deal evaluation. System prompt switches mode based on whether an offered price is detected. Suggestion chips cover both deals and market research queries. UI label: "Card Advisor · Prices · market trends · deal analysis". **Performance**: Both parse and advice Claude calls use `claude-haiku-4-5` for speed (6-9s response vs 15s with Sonnet). Two-tier price caching (in-memory + PostgreSQL) for repeat lookups. Frontend has 45s AbortController timeout with descriptive error messages. Disambiguation: when multiple cards match a query, backend returns `disambiguationCards[]` with inline image picker so user can tap the exact card.
