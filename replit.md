...
- **Market Value Estimation**: Multi-source price lookup: (1) pokemontcg.io API name-first search with TCGPlayer prices embedded, (2) TCGCSV API set-based fallback with comprehensive promo set aliases (SV, SWSH, SM, XY, BW, HGSS, DP, WOTC, ME promos all mapped to exact TCGPlayer set names). USD to GBP conversion for graded and raw cards, including estimated Grade 10 prices.
